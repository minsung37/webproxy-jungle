...
// 포트번호를 인자로 받아 클라이언트의 요청이 올 때마다 새로 연결 소켓을 만들어 doit() 함수 호출
...
void doit(int fd)
{
  ...
  // 1. 클라이언트가 rio로 보낸 request 읽기
  // 서버의 connfd를 연결
  Rio_readinitb(&rio, fd);
  // rio(==connfd)에 있는 응답 라인을 buf로 옮기기
  Rio_readlineb(&rio, buf, MAXLINE);
  printf("Requesst headers:\n");
  printf("%s", buf);
  sscanf(buf, "%s %s %s", method, uri, version);
  // 요청 method가 GET이 아니면 종료
  if (!(strcasecmp(method, "GET") == 0 || strcasecmp(method, "HEAD") == 0)) {
    clienterror(fd, method, "501", "Not implemented", "Tiny does not implement this method");
    return;
  }
  // 요청 라인을 뺀 나머지 요청 헤더들을 출력
  read_requesthdrs(&rio);

  // 2. 클라이언트 요청 라인에서 받아온 uri를 이용해 정적/동적 컨텐츠를 구분한다.
  ...
  // 못 넘기면 fail => 파일이 없다 => 404에러
  if (stat(filename, &sbuf) == -1) {
    clienterror(fd, filename, "404", "Not found", "Tiny couldn't find this file");
    return;
  }

  // 3. 컨텐츠의 유형(정적, 동적)을 파악한 후 각각의 서버에 보낸다.
  if (is_static) {
    // 일반 파일아닌경우 or 읽기권한 없는경우
    if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
      clienterror(fd, filename, "403", "Forbidden", "Tiny could't read the file");
      return;
    }
    // sbuf.st_size => Response header에 Content-length
    serve_static(fd, filename, sbuf.st_size, method);
  }
  else {
    // 일반 파일아닌경우 or 실행 권한이 없다
    if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {
      clienterror(fd, filename, "403", "Forbidden", "Tiny couldn't run the CGI program");
      return;
    }
    // 자식 프로세스 fork하고 그 후에 CGI 프로그램을 자식 프로세스에서 실행, 동적 컨텐츠 제공, 표준출력을 -> fd로 변환하여 결과를 클라이언트한테 전달
...
int parse_uri(char *uri, char *filename, char *cgiargs)
{
  ...
  // 예시 : GET /src/icon1.png HTTP/1.1  -> uri에 cgi-bin이 없다
  ...
  if (!strstr(uri, "cgi-bin")) {  // Static content, uri안에 "cgi-bin"과 일치하는 문자열이 있는지
    ...
    /* 
      ...
      ->
      cgiargs : 
      filename : ./src/icon1.png
    */
    return 1;
  }
  // request가 dynamic content에 대한 요청일 때: CGI argument를 추출한 다음 남아있는 URI 부분을 relative Linux filename으로 바꾼다.
  else {
    ptr = index(uri, '?');
...
void serve_static(int fd, char *filename, int filesize, char *method)
{
  ...
  // Send response headers to client 클라이언트에게 응답 헤더 보내기작성
  // 응답 라인과 헤더 작성
